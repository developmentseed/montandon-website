---
# https://vitepress.dev/reference/default-theme-home-page
layout: home

hero:
  name: "Montandon"
  text: "Global Crisis Data Bank"
  tagline: Montandon is the world’s largest disaster database, integrating natural hazards, impacts, and operational responses.
  actions:
    - theme: brand
      text: Get Started
      link: /get-started
    - theme: alt
      text: Get Involved
      link: /get-involved
    - theme: alt
      text: Learn More
      link: https://www.youtube.com/watch?v=BEWxqYfrQek&t

features:
  - title: Authoritative Data Sources
    details: Montandon sources over 1M records for over 100,000 events from 12 sources.
  - title: Unified Taxonomy
    details: Montandon uses a unified taxonomy to categorize and standardize disaster data across different sources.
  - title: STAC-compatable
    details: Montandon's data is organized in a SpatioTemporal Asset Catalog (STAC) format, making it easy to access and analyze geospatial data.
---
